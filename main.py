...
tf.disable_v2_behavior()  # Enable TensorFlow 1.x compatibility
...
parser.add_argument('--gpu_idx', dest='gpu_idx', default="0", help='GPU index')
parser.add_argument('--gpu_mem', dest='gpu_mem', type=float, default=0.5, help="0 to 1, GPU memory usage")
parser.add_argument('--phase', dest='phase', default='train', help='train or test')

parser.add_argument('--epoch', dest='epoch', type=int, default=100, help='number of total epochs')
...
parser.add_argument('--eval_every_epoch', dest='eval_every_epoch', type=int, default=20, help='evaluate and save checkpoints every # epoch')
...
def lowlight_train(lowlight_enhance):
    ...
    lr[20:] = lr[0] / 10.0  # Reduce learning rate after 20 epochs

    train_low_data = []
    train_high_data = []

    train_low_data_names = glob('./content/drive/My Drive/LOLI/LOLdataset/our485/low/*.png') + \
                           glob('./content/drive/My Drive/LOLI/LOLdataset/syn/low/*.png')
    train_low_data_names.sort()

    train_high_data_names = glob('./content/drive/My Drive/LOLI/LOLdataset/our485/high/*.png') + \
                            glob('./content/drive/My Drive/LOLI/LOLdataset/syn/high/*.png')
    ...
    eval_low_data = []
    eval_low_data_names = glob('./data/eval/low/*.*')
...
def lowlight_test(lowlight_enhance):
    ...
    for idx in range(len(test_low_data_names)):
        test_low_im = load_images(test_low_data_names[idx])
